...
export default function MainLayout() {
  ...
  const stats = useMemo(() => {
    ...
    const critical = seabins.filter((s) => s.contamination_risk === 'critical').length
          ...
          {!collapsed && (
            <div className="flex min-h-16 min-w-0 flex-1 items-center gap-2 pl-2">
              <img
                ...
                decoding="async"
              />
              <span className="shrink-0 text-[1.15rem] font-semibold tracking-tight text-teal-700">SeaTong</span>
            </div>
          )}
          {collapsed && (
            <img src={logoNav} alt="" className="h-10 w-10 shrink-0 object-contain" decoding="async" />
          )}
          <button
            type="button"
            ...
            <>
              ...
              <div className="mx-1.5 rounded-xl border border-slate-200/80 bg-slate-50/60 px-3 py-2.5">
                <div className="flex items-center justify-between text-[0.7rem]">
                  ...
                  <span className="font-semibold tabular-nums text-slate-800">{stats.active}</span>
                </div>
